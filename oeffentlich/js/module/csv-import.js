...
export function rendern() {
    ...
    container.innerHTML = `
        <div class="verwaltung">
            ...
            <!-- Synonym-Suche & Verwaltung -->
            <div class="karte" style="margin-top:24px" id="syn-bereich">
                <div class="karte__titel">
                    <span class="material-symbols-outlined" style="vertical-align:middle;font-size:20px;margin-right:4px">link</span>
                    ${t('csv_import.syn_titel')}
                </div>
                <div class="karte__inhalt">
                    <p style="margin:0 0 14px;color:var(--md-sys-color-on-surface-variant)">
                        ${t('csv_import.syn_beschreibung')}
                    </p>
                    ...
                    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:8px">
                        <input type="text" id="syn-eingabe" class="eingabe"
                               placeholder="${t('csv_import.syn_placeholder')}"
                               style="flex:1;min-width:160px;max-width:280px">
                        <button class="btn btn--umrandet" id="btn-syn-hinzufuegen" style="white-space:nowrap">
                            <span class="material-symbols-outlined" style="font-size:18px">add</span>
                            ${t('csv_import.syn_hinzufuegen')}
                        </button>
                        <button class="btn btn--gefuellt" id="btn-syn-suchen" disabled>
                            <span class="material-symbols-outlined" style="font-size:20px">search</span>
                            ${t('csv_import.syn_suchen')}
                        </button>
                    </div>

                    <!-- Such-Tags -->
                    <div id="syn-tags" style="display:flex;gap:6px;flex-wrap:wrap;min-height:28px;margin-bottom:4px">
                        <span style="font-size:13px;color:var(--md-sys-color-on-surface-variant);font-style:italic">${t('csv_import.syn_keine_begriffe')}</span>
                    </div>

                    <!-- Ergebnisse -->
                    <div id="syn-ergebnis" class="versteckt" style="margin-top:16px"></div>

                    <!-- Verknüpfen-Aktion -->
                    <div id="syn-aktion" class="versteckt" style="margin-top:12px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                        <button class="btn btn--umrandet" id="btn-syn-frag-ki" title="${t('csv_import.ki_title')}">
                            <span class="material-symbols-outlined" style="font-size:20px">psychology</span>
                            ${t('csv_import.frag_ki')}
                        </button>
                        <button id="btn-ki-fallback-toggle"
                                title="${t('csv_import.ki_fallback_title')}"
                            ...
                            <span class="material-symbols-outlined" id="ki-fallback-icon" style="font-size:16px">toggle_on</span>
                            ${t('csv_import.ki_fallback')}
                        </button>
                        <button class="btn btn--gefuellt" id="btn-syn-verknuepfen">
                            <span class="material-symbols-outlined" style="font-size:20px">link</span>
                            ${t('csv_import.syn_verknuepfen')}
                        </button>
                        <span id="syn-auswahl-hinweis" style="font-size:13px;color:var(--md-sys-color-on-surface-variant)"></span>
                    </div>
                </div>
            </div>

            <!-- Synonym-Vorschläge -->
            <div class="karte" style="margin-top:24px" id="syn-vorschlaege-bereich">
                <div class="karte__titel">
                    <span class="material-symbols-outlined" style="vertical-align:middle;font-size:20px;margin-right:4px">auto_awesome</span>
                    ${t('csv_import.syn_vorschlaege_titel')}
                </div>
                <div class="karte__inhalt">
                    <p style="margin:0 0 14px;color:var(--md-sys-color-on-surface-variant)">
                        ${t('csv_import.syn_vorschlaege_beschreibung')}
                    </p>


                    <button class="btn btn--umrandet" id="btn-syn-vorschlaege-laden">
                        <span class="material-symbols-outlined" style="font-size:20px">auto_awesome</span>
                        ${t('csv_import.syn_vorschlaege_laden')}
                    </button>
                    <div id="syn-vorschlaege-ergebnis" class="versteckt" style="margin-top:16px"></div>
                </div>
            </div>

            <!-- Synonyme verwalten -->
            <div class="karte" style="margin-top:24px" id="syn-liste-bereich">
                <div class="karte__titel">
                    <span class="material-symbols-outlined" style="vertical-align:middle;font-size:20px;margin-right:4px">manage_search</span>
                    ${t('csv_import.syn_liste_titel')}
                </div>
                <div class="karte__inhalt">
                    <p style="margin:0 0 14px;color:var(--md-sys-color-on-surface-variant)">
                        ${t('csv_import.syn_liste_beschreibung')}
                    </p>


                    <button class="btn btn--umrandet" id="btn-syn-liste-laden">
                        <span class="material-symbols-outlined" style="font-size:20px">refresh</span>
                        ${t('csv_import.syn_liste_laden')}
                    </button>
                    <div id="syn-liste-ergebnis" class="versteckt" style="margin-top:16px"></div>
                </div>
            </div>
...
function _syn_events_registrieren() {
    const eingabe = document.getElementById('syn-eingabe');
    const btnHinzufuegen = document.getElementById('btn-syn-hinzufuegen');
    const btnSuchen = document.getElementById('btn-syn-suchen');
    const btnVerknuepfen = document.getElementById('btn-syn-verknuepfen');

    eingabe?.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            _syn_tag_hinzufuegen();
        }
    });

    btnHinzufuegen?.addEventListener('click', _syn_tag_hinzufuegen);
    btnSuchen?.addEventListener('click', _syn_suchen);
    btnVerknuepfen?.addEventListener('click', _syn_verknuepfen);
    _ki_fallback_registrieren();

    document.getElementById('btn-syn-frag-ki')?.addEventListener('click', () => {
        const ausgewaehlt = [...document.querySelectorAll('.syn-auswahl-cb:checked')];
        const woerter = ausgewaehlt.map(cb => cb.dataset.schwedisch);
        if (woerter.length < 2) { fehler(t('csv_import.mind_2_auswaehlen')); return; }
        const deutsch = [...new Set(ausgewaehlt.map(cb => cb.dataset.deutsch))].filter(Boolean).join(' / ');
        const wortart = [...new Set(ausgewaehlt.map(cb => cb.dataset.wortart))].filter(Boolean).join(' / ');
        _frag_ki_anfragen(woerter, deutsch, wortart);
    });

    document.getElementById('btn-syn-vorschlaege-laden')?.addEventListener('click', _syn_vorschlaege_laden);
    document.getElementById('btn-syn-liste-laden')?.addEventListener('click', _syn_liste_laden);
}

function _syn_tag_hinzufuegen() {
    const eingabe = document.getElementById('syn-eingabe');
    const wert = eingabe?.value.trim();
    if (!wert) return;

    if (!_synSuchbegriffe.includes(wert)) {
        _synSuchbegriffe.push(wert);
        _syn_tags_rendern();
    }
...
function _syn_tags_rendern() {
    const container = document.getElementById('syn-tags');
    const btnSuchen = document.getElementById('btn-syn-suchen');
    if (!container) return;

    if (_synSuchbegriffe.length === 0) {
        container.innerHTML = '<span style="font-size:13px;color:var(--md-sys-color-on-surface-variant);font-style:italic">Noch keine Begriffe hinzugefügt</span>';
        if (btnSuchen) btnSuchen.disabled = true;
        return;
    }

    container.innerHTML = _synSuchbegriffe.map(term => `
        <span style="display:inline-flex;align-items:center;gap:4px;padding:4px 8px 4px 10px;
                     background:var(--md-sys-color-secondary-container);
                     color:var(--md-sys-color-on-secondary-container);
                     border-radius:16px;font-size:13px">
            ${esc(term)}
            <button class="syn-tag-remove" data-term="${esc(term)}"
    ...
    container.querySelectorAll('.syn-tag-remove').forEach(btn => {
        btn.addEventListener('click', () => {
            _synSuchbegriffe = _synSuchbegriffe.filter(term => term !== btn.dataset.term);
            _syn_tags_rendern();
        });
    });

    if (btnSuchen) btnSuchen.disabled = false;
}

async function _syn_suchen() {
    if (_synSuchbegriffe.length === 0) return;

    const btn = document.getElementById('btn-syn-suchen');
    const ergebnisDiv = document.getElementById('syn-ergebnis');
    const aktionDiv = document.getElementById('syn-aktion');
    ...
    if (ergebnisDiv) {
        ergebnisDiv.classList.remove('versteckt');
        lade_anzeige_rendern(ergebnisDiv);
    }
    if (aktionDiv) aktionDiv.classList.add('versteckt');

    const erg = await apiGet('vokabeln/synonym_suche.php', {
        begriffe: _synSuchbegriffe.join(','),
    });

    if (btn) {
        btn.disabled = false;
        btn.innerHTML = `<span class="material-symbols-outlined" style="font-size:20px">search</span> ${t('csv_import.syn_suchen')}`;
    }
    ...
    if (!erg.erfolg) {
        if (ergebnisDiv) {
            ergebnisDiv.innerHTML = `
                <div style="padding:12px;background:var(--md-sys-color-error-container);
                            border-radius:8px;color:var(--md-sys-color-on-error-container)">
                    <strong>Fehler:</strong> ${esc(erg.fehler?.nachricht || 'Unbekannter Fehler')}
                </div>`;
        }
        return;
    }

    _syn_ergebnis_rendern(erg.daten, ergebnisDiv);
}

function _syn_ergebnis_rendern(vokabeln, container) {
    if (!container) return;

    if (vokabeln.length === 0) {
        container.innerHTML = `
            <div style="display:flex;align-items:center;gap:8px;color:var(--md-sys-color-on-surface-variant);padding:8px 0">
                <span class="material-symbols-outlined">search_off</span>
                <span>${t('csv_import.keine_vokabeln_fuer', {begriffe: esc(_synSuchbegriffe.join(', '))})}</span>
            </div>`;
        return;
    }

    let html = `
        <div style="font-size:13px;color:var(--md-sys-color-on-surface-variant);margin-bottom:8px">
            ${t('csv_import.vokabeln_gefunden', {n: vokabeln.length})}
        </div>
        <div class="verwaltung-tabelle-wrapper" style="max-height:400px;overflow-y:auto;margin-bottom:8px">
        <table class="verwaltung-tabelle verwaltung-tabelle--kompakt">
            <thead><tr>
                <th style="width:36px">
                    <input type="checkbox" id="syn-alle-auswaehlen" title="${t('csv_import.alle_auswaehlen')}">
                </th>
                <th>${t('csv_import.th_schwedisch')}</th>
                <th>${t('csv_import.th_wortart')}</th>
                <th>${t('csv_import.th_deutsch')}</th>
                <th>${t('csv_import.th_synonyme_sv')}</th>
            </tr></thead>
            <tbody>
    `;

    for (const v of vokabeln) {
        const svSynonyme = v.synonyme.filter(s => s.sprache === 'sv').map(s => s.synonym);
        html += `
            <tr>
                <td>
                    <input type="checkbox" class="syn-auswahl-cb"
                           data-id="${v.id}" data-schwedisch="${esc(v.schwedisch)}"
                           data-deutsch="${esc(v.deutsch)}" data-wortart="${esc(v.wortart)}">
                </td>
                <td><strong>${esc(v.schwedisch)}</strong></td>
                <td><span class="tag">${esc(v.wortart)}</span></td>
                <td>${esc(v.deutsch)}</td>
                <td style="font-size:12px;color:var(--md-sys-color-on-surface-variant)">
                    ${svSynonyme.length > 0
                        ? svSynonyme.map(s => `<span class="tag" style="font-size:11px">${esc(s)}</span>`).join(' ')
                        : '<em>–</em>'}
                </td>
            </tr>
        `;
    ...
    const aktionDiv = document.getElementById('syn-aktion');
    const hinweis = document.getElementById('syn-auswahl-hinweis');
    const checkboxen = container.querySelectorAll('.syn-auswahl-cb');
    const alleCheckbox = document.getElementById('syn-alle-auswaehlen');

    const _auswahl_aktualisieren = () => {
        const ausgewaehlt = [...checkboxen].filter(cb => cb.checked);
        const anzahl = ausgewaehlt.length;

        if (aktionDiv) {
            if (anzahl >= 2) {
                aktionDiv.classList.remove('versteckt');
            } else {
                aktionDiv.classList.add('versteckt');
            }
        }
        if (hinweis) hinweis.textContent = anzahl >= 2 ? t('csv_import.n_ausgewaehlt', {n: anzahl}) : '';

        if (alleCheckbox) {
            alleCheckbox.indeterminate = anzahl > 0 && anzahl < checkboxen.length;
            alleCheckbox.checked = anzahl > 0 && anzahl === checkboxen.length;
        }
    };

    checkboxen.forEach(cb => cb.addEventListener('change', _auswahl_aktualisieren));

    alleCheckbox?.addEventListener('change', () => {
        checkboxen.forEach(cb => cb.checked = alleCheckbox.checked);
        _auswahl_aktualisieren();
    });
}

async function _syn_verknuepfen() {
    const ausgewaehlt = [...document.querySelectorAll('.syn-auswahl-cb:checked')];

    if (ausgewaehlt.length < 2) {
        fehler(t('csv_import.mind_2_auswaehlen'));
        return;
    }

    const ids = ausgewaehlt.map(cb => parseInt(cb.dataset.id, 10));
    const woerter = ausgewaehlt.map(cb => cb.dataset.schwedisch).join(', ');

    const btn = document.getElementById('btn-syn-verknuepfen');
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = `<span class="material-symbols-outlined" style="font-size:20px">hourglass_empty</span> ${t('csv_import.verknuepfe')}`;
    }

    const erg = await apiPost('vokabeln/synonyme_verknuepfen.php', { vokabel_ids: ids });

    if (btn) {
        btn.disabled = false;
        btn.innerHTML = `<span class="material-symbols-outlined" style="font-size:20px">link</span> ${t('csv_import.ausgewaehlte_verknuepfen')}`;
    }

    if (erg.erfolg) {
        const neu = erg.daten?.verknuepft ?? 0;
        erfolg(neu > 0
            ? t('csv_import.syn_verknuepft_erfolg', {n: neu, woerter})
            : t('csv_import.syn_bereits_verknuepft')
        );
        // Ergebnisse auffrischen, damit neue Synonyme sichtbar werden
        _syn_suchen();
    } else {
        apiFehlerAnzeigen(erg);
    }
...
const _KI_FALLBACK_KEY = 'vt_ki_fallback';

function _ki_fallback_aktiv() {
    return localStorage.getItem(_KI_FALLBACK_KEY) !== 'aus';
}

function _ki_fallback_toggle_aktualisieren() {
    const aktiv  = _ki_fallback_aktiv();
    const btn    = document.getElementById('btn-ki-fallback-toggle');
    const icon   = document.getElementById('ki-fallback-icon');
    if (!btn) return;
    icon.textContent      = aktiv ? 'toggle_on' : 'toggle_off';
    btn.style.color       = aktiv
        ? 'var(--md-sys-color-primary)'
        : 'var(--md-sys-color-on-surface-variant)';
    btn.title = aktiv
        ? t('csv_import.ki_fallback_aktiv')
        : t('csv_import.ki_fallback_inaktiv');
}

function _ki_fallback_registrieren() {
    _ki_fallback_toggle_aktualisieren();
    document.getElementById('btn-ki-fallback-toggle')?.addEventListener('click', () => {
        localStorage.setItem(_KI_FALLBACK_KEY, _ki_fallback_aktiv() ? 'aus' : 'an');
        _ki_fallback_toggle_aktualisieren();
    });
}

// ---- KI-Befragung (Gemini primär / Claude Fallback via PHP-Proxy) ----

async function _frag_ki_anfragen(woerter, deutsch, wortart) {
    // Modal sofort mit Ladeanzeige öffnen
    _ki_modal_zeigen(woerter, `
        <div style="display:flex;align-items:center;gap:12px;padding:16px 0;
                    color:var(--md-sys-color-on-surface-variant)">
            <span class="material-symbols-outlined">hourglass_empty</span>
            <span>${t('csv_import.ki_wird_befragt')}</span>
        </div>`);

    const erg = await apiPost('ki/synonym_bewertung.php', { woerter, deutsch, wortart, mit_fallback: _ki_fallback_aktiv() });

    if (erg.erfolg) {
        _ki_modal_inhalt_setzen(_ki_markdown_zu_html(erg.daten.antwort));
        // Quellen-Badge aktualisieren
        const quelle = erg.daten.quelle || '';
        const badge  = document.getElementById('ki-modal-quelle');
        if (badge) {
            const label = quelle === 'claude' ? '✦ Claude' : '✦ Gemini';
            badge.textContent = label;
            badge.style.display = 'inline-block';
        }
    } else {
        _ki_modal_inhalt_setzen(`
            <div style="color:var(--md-sys-color-error);padding:8px 0">
                <strong>${t('csv_import.fehler_label')}:</strong> ${esc(erg.fehler?.nachricht || t('csv_import.unbekannter_fehler'))}
            </div>`);
    }
}

function _ki_modal_zeigen(woerter, inhalt) {
    document.getElementById('ki-bewertungs-modal')?.remove();

    const woerterHtml = woerter.map(w => `<strong>${esc(w)}</strong>`).join(' + ');
    const modal = document.createElement('div');
    modal.id = 'ki-bewertungs-modal';
    modal.style.cssText =
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;
                ...
                <div style="display:flex;align-items:center;gap:8px">
                    <span class="material-symbols-outlined"
                          style="color:var(--md-sys-color-primary)">psychology</span>
                    <h3 style="margin:0;font-size:18px">${t('csv_import.ki_bewertung_titel')}</h3>
                    <span id="ki-modal-quelle"
                          style="display:none;font-size:11px;font-weight:600;letter-spacing:.4px;
                                 padding:2px 8px;border-radius:99px;
                                 background:var(--md-sys-color-secondary-container);
                                 color:var(--md-sys-color-on-secondary-container)"></span>
                </div>
                <button class="btn-icon ki-modal-close-btn" title="${t('allgemein.schliessen')}">
                    <span class="material-symbols-outlined">close</span>
                </button>
            </div>
            <div style="font-size:13px;color:var(--md-sys-color-on-surface-variant);
                        margin-bottom:14px;flex-shrink:0">
                ${t('csv_import.ki_geprueft')}: ${woerterHtml}
            </div>
            <div id="ki-modal-inhalt"
                 style="overflow-y:auto;flex:1;line-height:1.7;font-size:14px">
                ${inhalt}
            </div>
        </div>`;
    ...
    modal.querySelector('.ki-modal-close-btn')?.addEventListener('click', () => modal.remove());
...
function _ki_modal_inhalt_setzen(html) {
    const el = document.getElementById('ki-modal-inhalt');
    if (el) el.innerHTML = html;
}

function _ki_markdown_zu_html(text) {
    // Inline-Formatierung (Bold vor Italic um Kollision zu vermeiden)
    function inline(s) {
        s = s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        s = s.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
        s = s.replace(/\*(.+?)\*/g,     '<em>$1</em>');
        s = s.replace(/`([^`]+)`/g,     '<code style="background:var(--md-sys-color-surface-container);' +
            'padding:1px 5px;border-radius:4px;font-size:12px">$1</code>');
        return s;
    }

    // Zeilenbasierte Verarbeitung — verhindert Regex-Kollision zwischen * (Bullet) und * (Italic)
    ...
    for (const rawLine of lines) {
        ...
        // Überschriften
        if (line.startsWith('### ')) { closeList(); out.push(`<h4 style="margin:12px 0 3px;font-size:14px">${inline(line.slice(4))}</h4>`); continue; }
        if (line.startsWith('## '))  { closeList(); out.push(`<h3 style="margin:14px 0 4px;font-size:15px">${inline(line.slice(3))}</h3>`); continue; }
        if (line.startsWith('# '))   { closeList(); out.push(`<h2 style="margin:16px 0 6px">${inline(line.slice(2))}</h2>`); continue; }

        // Nummerierte Liste (1. / 1) / 2. etc.) — Gemini nutzt das bei unserem Prompt
        const olM = line.match(/^(\d+)[.)]\s+(.+)$/);
        if (olM) {
            if (!inOl) { closeList(); out.push('<ol style="margin:6px 0;padding-left:22px">'); inOl = true; }
            out.push(`<li style="margin:5px 0;line-height:1.6">${inline(olM[2])}</li>`);
            continue;
        }
        ...
        if (ulM) {
            if (!inUl) { closeList(); out.push('<ul style="margin:4px 0;padding-left:18px">'); inUl = true; }
        ...
        // Normaler Absatz
        closeList();
        out.push(`<p style="margin:5px 0;line-height:1.65">${inline(line)}</p>`);
    }
...
// ---- Automatische Synonym-Vorschläge ----

async function _syn_vorschlaege_laden() {
    const btn = document.getElementById('btn-syn-vorschlaege-laden');
    const container = document.getElementById('syn-vorschlaege-ergebnis');

    if (btn) {
        btn.disabled = true;
        btn.innerHTML = `<span class="material-symbols-outlined" style="font-size:20px">hourglass_empty</span> ${t('csv_import.lade')}`;
    }
    if (container) {
        container.classList.remove('versteckt');
        lade_anzeige_rendern(container);
    ...
    const erg = await apiGet('vokabeln/synonym_vorschlaege.php');

    if (btn) {
        btn.disabled = false;
        btn.innerHTML = `<span class="material-symbols-outlined" style="font-size:20px">auto_awesome</span> ${t('csv_import.vorschlaege_neu_laden')}`;
    }
    ...
    _syn_vorschlaege_rendern(erg.daten, container);
}

function _syn_vorschlaege_rendern(vorschlaege, container) {
    if (!container) return;

    if (vorschlaege.length === 0) {
        container.innerHTML = `
            <div style="display:flex;align-items:center;gap:8px;color:var(--md-sys-color-tertiary);padding:8px 0">
                <span class="material-symbols-outlined">check_circle</span>
                <span>${t('csv_import.keine_vorschlaege')}</span>
            </div>`;
        return;
    }
    ...
    let html = `
        <div style="font-size:13px;color:var(--md-sys-color-on-surface-variant);margin-bottom:8px">
            <strong>${vorschlaege.length}</strong> ${t('csv_import.paare_gefunden')}:
        </div>
        <div class="verwaltung-tabelle-wrapper" style="max-height:400px;overflow-y:auto">
        <table class="verwaltung-tabelle verwaltung-tabelle--kompakt">
            <thead><tr>
                <th>${t('csv_import.vokabel_a')}</th>
                <th>${t('csv_import.vokabel_b')}</th>
                <th>${t('csv_import.deutsch')}</th>
                <th>${t('csv_import.wortart')}</th>
                <th style="width:180px">${t('csv_import.aktion')}</th>
            </tr></thead>
            <tbody id="syn-vorschlaege-tbody">
    `;

    for (const v of vorschlaege) {
        html += `
            <tr data-id-a="${v.id_a}" data-id-b="${v.id_b}">
                <td><strong>${esc(v.schwedisch_a)}</strong></td>
                <td><strong>${esc(v.schwedisch_b)}</strong></td>
                <td>${esc(v.deutsch)}</td>
                <td><span class="tag">${esc(v.wortart)}</span></td>
                <td style="white-space:nowrap">
                    <button class="btn btn--klein btn--umrandet syn-vorschlag-frag-ki"
                            data-schwedisch-a="${esc(v.schwedisch_a)}" data-schwedisch-b="${esc(v.schwedisch_b)}"
                            data-deutsch="${esc(v.deutsch)}" data-wortart="${esc(v.wortart)}"
                            style="margin-right:4px" title="KI befragen">
                        <span class="material-symbols-outlined" style="font-size:16px">psychology</span>
                        ${t('csv_import.frag_ki')}
                    </button>
                    <button class="btn btn--klein btn--gefuellt syn-vorschlag-bestaetigen"
                            data-id-a="${v.id_a}" data-id-b="${v.id_b}"
                            data-schwedisch-a="${esc(v.schwedisch_a)}" data-schwedisch-b="${esc(v.schwedisch_b)}"
                            style="margin-right:4px">
                        <span class="material-symbols-outlined" style="font-size:16px">check</span>
                        ${t('csv_import.verknuepfen')}
                    </button>
                    <button class="btn btn--klein btn--umrandet syn-vorschlag-ignorieren">
                        <span class="material-symbols-outlined" style="font-size:16px">close</span>
                        ${t('csv_import.ignorieren')}
                    </button>
                </td>
            </tr>
        `;
    ...
    // Events für alle Zeilen
    container.querySelectorAll('tr[data-id-a]').forEach(row => {
        row.querySelector('.syn-vorschlag-frag-ki')?.addEventListener('click', (e) => {
            const btn = e.currentTarget;
            _frag_ki_anfragen([btn.dataset.schwedischA, btn.dataset.schwedischB], btn.dataset.deutsch, btn.dataset.wortart);
        });

        row.querySelector('.syn-vorschlag-bestaetigen')?.addEventListener('click', async (e) => {
            const btn = e.currentTarget;
            const idA = parseInt(btn.dataset.idA, 10);
            const idB = parseInt(btn.dataset.idB, 10);
            const wA  = btn.dataset.schwedischA;
            const wB  = btn.dataset.schwedischB;

            btn.disabled = true;
            btn.innerHTML = '<span class="material-symbols-outlined" style="font-size:16px">hourglass_empty</span>';

            const erg = await apiPost('vokabeln/synonyme_verknuepfen.php', { vokabel_ids: [idA, idB] });

            if (erg.erfolg) {
                erfolg(t('csv_import.syn_paar_verknuepft', {a: wA, b: wB}));
                row.remove();
                // Zähler in Überschrift aktualisieren
                const tbody = document.getElementById('syn-vorschlaege-tbody');
                const verbleibend = tbody?.querySelectorAll('tr').length ?? 0;
                const info = container.querySelector('div[style*="margin-bottom:8px"]');
                if (info) info.innerHTML = `<strong>${verbleibend}</strong> ${t('csv_import.paare_verbleibend')}:`;
                if (verbleibend === 0) _syn_vorschlaege_rendern([], container);
            } else {
                btn.disabled = false;
                btn.innerHTML = `<span class="material-symbols-outlined" style="font-size:16px">check</span> ${t('csv_import.verknuepfen')}`;
                apiFehlerAnzeigen(erg);
            }
        });

        row.querySelector('.syn-vorschlag-ignorieren')?.addEventListener('click', () => {
            row.remove();
            const tbody = document.getElementById('syn-vorschlaege-tbody');
            const verbleibend = tbody?.querySelectorAll('tr').length ?? 0;
            const info = container.querySelector('div[style*="margin-bottom:8px"]');
...
// ---- Synonym-Liste & Trennen ----

async function _syn_liste_laden() {
    const btn = document.getElementById('btn-syn-liste-laden');
    const container = document.getElementById('syn-liste-ergebnis');
    ...
    const erg = await apiGet('vokabeln/synonym_liste.php');

    if (btn) {
        btn.disabled = false;
        btn.innerHTML = `<span class="material-symbols-outlined" style="font-size:20px">refresh</span> ${t('csv_import.synonyme_aktualisieren')}`;
    }
    ...
    _syn_liste_rendern(erg.daten, container);
}

function _syn_liste_rendern(paare, container) {
    if (!container) return;
    ...
    if (paare.length === 0) {
        container.innerHTML = `
            <div style="display:flex;align-items:center;gap:8px;color:var(--md-sys-color-on-surface-variant);padding:8px 0">
                ...
                <span>${t('csv_import.keine_synonyme')}</span>
            </div>`;
        return;
    }
    ...
    let html = `
        <div style="font-size:13px;color:var(--md-sys-color-on-surface-variant);margin-bottom:8px">
            <strong>${paare.length}</strong> ${t('csv_import.synonym_paare')}:
        </div>
        <div class="verwaltung-tabelle-wrapper" style="max-height:500px;overflow-y:auto">
        <table class="verwaltung-tabelle verwaltung-tabelle--kompakt">
            <thead><tr>
                <th>${t('csv_import.vokabel_a')}</th>
                <th>${t('csv_import.richtung')}</th>
                <th>${t('csv_import.vokabel_b')}</th>
                <th>${t('csv_import.deutsch')}</th>
                <th>${t('csv_import.wortart')}</th>
                <th style="width:100px">${t('csv_import.aktion')}</th>
            </tr></thead>
            <tbody>
    `;

    for (const p of paare) {
        const richtung = p.bidirektional ? '↔' : '→';
        const richtungTitle = p.bidirektional ? t('csv_import.bidirektional') : t('csv_import.einseitig');
        html += `
            <tr>
                <td><strong>${esc(p.schwedisch_a)}</strong></td>
                <td style="text-align:center;font-size:16px" title="${richtungTitle}">${richtung}</td>
                <td><strong>${esc(p.schwedisch_b)}</strong></td>
                <td style="color:var(--md-sys-color-on-surface-variant)">${esc(p.deutsch)}</td>
                <td><span class="tag">${esc(p.wortart)}</span></td>
                <td>
                    <button class="btn btn--klein btn--umrandet syn-liste-trennen"
                            data-id-a="${p.id_a}"
                            data-id-b="${p.id_b ?? ''}"
                            data-synonym-text="${esc(p.synonym_text)}"
                            data-schwedisch-a="${esc(p.schwedisch_a)}"
                            data-schwedisch-b="${esc(p.schwedisch_b)}"
                            style="color:var(--md-sys-color-error)">
                        <span class="material-symbols-outlined" style="font-size:16px">link_off</span>
                        ${t('csv_import.trennen')}
                    </button>
                </td>
            </tr>
        `;
    ...
    container.querySelectorAll('.syn-liste-trennen').forEach(btn => {
        btn.addEventListener('click', async () => {
            const idA   = parseInt(btn.dataset.idA, 10);
            const idB   = btn.dataset.idB ? parseInt(btn.dataset.idB, 10) : null;
            const wA    = btn.dataset.schwedischA;
            const wB    = btn.dataset.schwedischB;
            const text  = btn.dataset.synonymText;
            ...
            const body = idB
                ? { vokabel_id_a: idA, vokabel_id_b: idB }
                : { vokabel_id_a: idA, synonym_text: text };

            const erg = await apiPost('vokabeln/synonyme_trennen.php', body);

            if (erg.erfolg) {
                erfolg(t('csv_import.verknuepfung_aufgehoben', {a: wA, b: wB}));
                row?.remove();
                // Zähler aktualisieren
                const verbleibend = container.querySelectorAll('tbody tr').length;
                const info = container.querySelector('div[style*="margin-bottom:8px"]');
                if (info) info.innerHTML = `<strong>${verbleibend}</strong> ${t('csv_import.synonym_paare')}:`;
                if (verbleibend === 0) _syn_liste_rendern([], container);
            } else {
                btn.disabled = false;
                btn.innerHTML = `<span class="material-symbols-outlined" style="font-size:16px">link_off</span> ${t('csv_import.trennen')}`;
                apiFehlerAnzeigen(erg);
            }
        });
    });
